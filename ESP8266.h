...
#endif
    friend class ESP8266_Client;
};

class WiFi_Print : public Print {
  protected:
    uint8_t m_mux_id;
    ESP8266 *m_wifi;
  public:
    WiFi_Print(ESP8266 *wifi, uint8_t mux_id): m_wifi(wifi), m_mux_id(mux_id) { }
    virtual size_t write(uint8_t data)
    {
    	m_wifi->send(m_mux_id, (const uint8_t *)data, 1);
      return 1;
    }
    uint32_t recv(uint8_t *buffer, uint32_t buffer_size, uint32_t timeout = 1000)
    {
      return m_wifi->recv(m_mux_id, buffer, buffer_size, timeout);
    }
    size_t write(const char *str)
    {
      size_t size = strlen(str);
      m_wifi->send(m_mux_id, (const uint8_t *)str, size);
      return size;
    }
    size_t write(const uint8_t *buffer, size_t size)
    {
      m_wifi->send(m_mux_id, buffer, size);
      return size;
    }
};
class ESP8266_Client : public Client, public WiFi_Print
{
  private:
    uint8_t m_bConnected;
  public:
  	int Timeout;
    ESP8266_Client(ESP8266 *wifi, uint8_t mux_id): WiFi_Print(wifi,mux_id)
    {
    	m_bConnected = false;
    	Timeout = 5000;
    }
    virtual int connect(IPAddress ip, uint16_t port)
    {
      ...
      if (m_wifi->createTCP(m_mux_id, s, port))
      {
      	m_bConnected = 1;
        return 1;
      }
    ...
    {
      if (m_wifi->createTCP(m_mux_id, host, port))
      {
      	m_bConnected = 1;
        return 1;
      }
    ...
    {
    	return m_wifi->m_puart->available();
    }
    virtual int read()
    {
    	int ret = -1;
    	uint8_t c;
    	if(recv(&c, 1, Timeout) == 1)
    	{
    			ret = c;
    	}
    	return ret;	
